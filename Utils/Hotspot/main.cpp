...
HotSpotFinder::HotSpotFinder(char* boundFile, double delta=70, double r=100)
{
    ...
    if(bf==NULL) { cout << " No bound file\n"; exit(1); }
    (*bf)>>Xmin>>Xmax>>Ymin>>Ymax;
    bf->close();
    cout<<"Read from bound file:\n";
    cout<<"Xmin="<<Xmin<<"\t Xmax="<<Xmax<<"\n"<<"Ymin="<<Ymin<<"\t Ymax="<<Ymax<<endl<<endl;

    Xmin=floor(Xmin/Delta)*Delta;  Xmax=ceil(Xmax/Delta)*Delta;  Xsize=(int)ceil((Xmax-Xmin)/Delta);
    Ymin=floor(Ymin/Delta)*Delta;  Ymax=ceil(Ymax/Delta)*Delta;  Ysize=(int)ceil((Ymax-Ymin)/Delta);
...
void HotSpotFinder::printHotSpot(Cluster& HotSpot)
{
    ...
    cout<<"Total time in cluster:"<< HotSpot.sumTime<<"sec \t Number of waypoints:"<<(HotSpot.waypoint).size()<<endl;
    for(unsigned int i=0; i<(HotSpot.waypoint).size(); i++)
