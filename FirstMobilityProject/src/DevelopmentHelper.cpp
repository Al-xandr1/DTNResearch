...
public:
    static char* createFileName(char* buffer, int numberOfExperiment,
        ...
        if (index >= 0) {
            ...
            result = strcat(
                    strcat(strcat(result, "_"), ((postfix.str()).c_str())),
                    FILE_TYPE);
        } else {
    ...
    static int countMaxValue(list<int> queueSizePoints) {
        int maxQueueSize = 0;
        list<int>::iterator iterSize;
        for (iterSize = queueSizePoints.begin();
                iterSize != queueSizePoints.end(); ++iterSize) {
